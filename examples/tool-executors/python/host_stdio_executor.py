...
SESSION_ID = "sess_python_stdio_executor"
    ...
    send(
        {
            ...
            "payload": {
                "executor_id": "python-stdio-example",
                "display_name": "Python stdio example",
        ...
        if message_type == "execute_tool":
            ...
            result = {
                "content": [
                    {
                        "type": "text",
                        "text": f"python stdio executor received: {arguments.get('message', '')}",
                    }
                ],
                "isError": False,
            }
            send(execution_result(message, result))
            continue

        if message_type in {"heartbeat", "shutdown", "disconnect_notice"}:
            send(status_response(message))
